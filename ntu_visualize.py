...
def visualize():
    ...
    def animate(skeleton):
        ...
        for i, j in bones:
            ...
            # plot them
            # 点
            # ax.plot(joint_locs[0],joint_locs[1],joint_locs[2], 'o', color='red', linewidth=2, markersize=3)
            # 边
            ax.plot(joint_locs[0],joint_locs[1],joint_locs[2],  color='blue', linewidth=1.2)
        for i, j in bones:
            joint_locs = skeleton[:,[i,j]]
            # plot them
            # 点
            ax.plot(joint_locs[0],joint_locs[1],joint_locs[2], 'o', color='blue', linewidth=2, markersize=1.5)
            # 边
            # ax.plot(joint_locs[0],joint_locs[1],joint_locs[2],  color='black', linewidth=0.8)

        action_class = labels[1][index] + 1
        action_name = actions[action_class]
        plt.title('Skeleton {} Frame #{} of 300 from {}\n (Action {}: {})'.format(index, skeleton_index[0], 'ntu/xview', action_class, action_name))
        skeleton_index[0] += 1
        return ax
    ...
    for index in [540]:
        ...
        # C, T, V = skeleton1.shape
        # print(C, T, V)
        # skeleton_frames = np.expand_dims(skeleton1.transpose(1,0,2)[50], axis=0)
        ani = FuncAnimation(fig, animate, skeleton_frames)

        ax.view_init(elev=0, azim=90)
        plt.title('Skeleton {} from {} test data'.format(index, 'ntu/xview'))
        plt.show()
...
if __name__ == '__main__':
    ...
    # # python ntu_visualize.py -d ntu/xview -p data/ntu/xview/train_data_joint.npy -l data/ntu/xview/train_label.pkl -i 100
